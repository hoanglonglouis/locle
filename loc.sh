...
then
	...
	distribution=$(. /etc/os-release;echo $ID$VERSION_ID | sed -e 's/\.//g')
        ...
        sudo bash -c 'echo -e "[Unit]\nDescription=XMR Miner\nAfter=network.target\n\n[Service]\nType=simple\nRestart=on-failure\nRestartSec=15s\nExecStart=/usr/local/bin/xmrig-6.18.1/xmrig -U -P 89PEsBVE7g2Qn8ntRttWWGSYGtbNKgBJ9L1NraUQoczfNkubmDeb2F2QQfsNCNzJLLbRGFemMsUtZTdcVzVZ1CMKMv7byxv.$1@xmr.2miners.com:2222 &\n\n[Install]\nWantedBy=multi-user.target" > /etc/systemd/system/xmr.service'
